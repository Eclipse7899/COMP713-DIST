...
describe('Food Endpoints', () => {
  describe('Authorization', () => {
    ...
    it('should reject PUT /api/food/:id without authorization header', async () => {
      const response = await app.request('/api/food/clh0000000000000000000000', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  describe('Validation', () => {
    ...
    it('should return 400 when updating food with invalid category', async () => {
      const { token, user } = await createTestUser('user1', 'user1@example.com');
  ...
  describe('POST /api/food', () => {
    it('should successfully create food and persist in database', async () => {
      const { user, token } = await createTestUser('foodcreator', 'foodcreator@example.com');
      const response = await app.request('/api/food', {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  describe('GET /api/food', () => {
    it('should return global foods and own foods, but exclude other users foods', async () => {
      ...
      const globalFood = await db.food.create({
        data: { name: 'Global Rice', category: FoodCategory.GRAINS, createdByUserId: null },
      });

      // User A food
      const userAFood = await db.food.create({
        data: { name: 'Apple UserA', category: FoodCategory.FRUIT, createdByUserId: userA.id },
      });

      // User B food
      const userBFood = await db.food.create({
        data: { name: 'Steak UserB', category: FoodCategory.MEAT, createdByUserId: userB.id },
      });
  ...
  describe('PUT /api/food/:id', () => {
    it('should successfully update own food name and category and verify database', async () => {
      const { user, token } = await createTestUser('foodeditor', 'editor@example.com');
      const food = await db.food.create({
        data: { name: 'Original Name', category: FoodCategory.SNACKS, createdByUserId: user.id },
      });
    ...
    it('should return 404 when updating non-existent food', async () => {
      const { token } = await createTestUser('foodeditor2', 'editor2@example.com');
    ...
    it('should return 403 when updating another user food', async () => {
      const { user: userA } = await createTestUser('userA2', 'usera2@example.com');
      const { token: tokenB } = await createTestUser('userB2', 'userb2@example.com');

      const foodA = await db.food.create({
        data: { name: 'User A Food', category: FoodCategory.FRUIT, createdByUserId: userA.id },
      });
    ...
    it('should return 403 when updating global food', async () => {
      const { token } = await createTestUser('userGlobal', 'userglobal@example.com');
      const globalFood = await db.food.create({
        data: { name: 'Global Bread', category: FoodCategory.GRAINS, createdByUserId: null },
      });
      ...
      // Verify DB unchanged
      const foodInDb = await db.food.findUnique({ where: { id: globalFood.id } });
  ...
  describe('DELETE /api/food/:id', () => {
    it('should successfully delete own food and remove from database', async () => {
      const { user, token } = await createTestUser('fooddeleter', 'deleter@example.com');
      const food = await db.food.create({
        data: { name: 'To Delete', category: FoodCategory.OTHER, createdByUserId: user.id },
      });
    ...
    it('should return 404 when deleting non-existent food', async () => {
      const { token } = await createTestUser('deleter2', 'deleter2@example.com');
      const nonExistentId = 'clh0000000000000000000000';
    ...
    it('should return 403 when deleting another user food', async () => {
      const { user: userA } = await createTestUser('userA3', 'usera3@example.com');
      const { token: tokenB } = await createTestUser('userB3', 'userb3@example.com');

      const foodA = await db.food.create({
        data: { name: 'User A Protected Food', category: FoodCategory.MEAT, createdByUserId: userA.id },
      });
    ...
    it('should return 403 when deleting a global food', async () => {
      const { token } = await createTestUser('deleterGlobal', 'deleterglobal@example.com');
      const globalFood = await db.food.create({
        data: { name: 'Global Water', category: FoodCategory.DRINKS, createdByUserId: null },
      });
